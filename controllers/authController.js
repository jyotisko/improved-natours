...
const createSendToken = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    ...
    // secure: true, // Cookie will be sent only in HTTPS connections
    httpOnly: true // Makes sure that the cookie can not be modified by the browser (protects from XSS)
  };
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!currentUser) return next(new AppError('The user beloging to this token no longer exists.', 401));

  // 4) Check if user changed password, after token was issued
  if (currentUser.passwordChangedAfter(decoded.iat)) return next(new AppError('User recently changed password! Please login again.', 401));
